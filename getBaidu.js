 ...
 * @LastEditTime: 2020-11-22 12:12:49
 * @LastEditors: 左太宇
 * @message: 获取百度实时热点
 */
...
async function mian() {
  ...
  await page.goto("http://top.baidu.com/buzz?b=1");
  console.log("页面加载完毕");

  await page.waitForSelector(".list-table"); // 等待元素加载之后，否则获取不到异步加载的元素
  await page.screenshot({ path: "example.png" });
  const data = await page.$$eval(".list-table .list-title", (e) => {
    const ctn = e.map((v, i) => {
      let obj = {
        id: i,
        title: v.innerText,
  ...
  var file = path.join(__dirname, "data/baidu.json");
